...
export function PrivacyScreen({ onBack, standalone }) {
  return h('.screen',
    ...
    h('.flow.pad-bottom', h('.wrap',
      ...
      h('.card',
        ...
        h('.kv', h('span', 'Де живе запис'), h('b', 'У вкладці браузера')),
      ...
      h('ul.list.tiny',
        ...
        h('li', 'Не передаємо нічого третім сторонам — у цій версії взагалі немає зовнішніх запитів.'),
      ),
  ...
  root.append(
    ...
    h('.flow.pad-bottom', h('.wrap',
      ...
      h('.card',
        ...
        h('.kv', h('span', 'Подій у локальному журналі'), h('b', String(evCount))),
...
export function ProgressScreen({ state, onBack }) {
  ...
  return h('.screen',
    ...
    h('.flow.pad-bottom', h('.wrap',
      ...
      h('p.tiny', 'Дні 4–7 ще готуються. У цій тестовій версії наскрізно реалізований тільки день 3 — щоб перевірити головне: чи люди справді говорять уголос.'),
    )),
  );
}
